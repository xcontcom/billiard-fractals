...
The binary sequence generated by <img src="images/part4_florksqrt2mod2.svg" alt="$\left\lfloor k \sqrt{2} \right\rfloor \bmod 2$"> can be viewed geometrically as a rotation around the unit circle. At each step <img src="images/part4_k.svg" alt="$k$">, we rotate by an irrational angle (in this case, <img src="images/part4_sqrt2.svg" alt="$\sqrt{2}$">) and assign a value of 0 or 1 depending on which half of the circle the point falls into.

The alternative formulation <img src="images/part4_sinpiksqrt2more.svg" alt="$\sin\left( \pi k \sqrt{2} \right) > 0$"> produces the same result: the sine function is positive in one half of the circle and negative in the other, effectively encoding the same threshold.

![Picture](images/sin.png)

This circular diagram shows the orbit of <img src="images/part4_ksqrt2mod1.svg" alt="$k \sqrt{2} \bmod 1$"> projected onto the unit circle. The points rotate continuously around the circumference, never repeating. Each crossing of the horizontal axis (i.e. sign change of <img src="images/part4_sinpiksqrt2.svg" alt="$\sin\left( \pi k \sqrt{2} \right)$">) corresponds to a binary switch in the sequence.

The result is a **Sturmian sequence** - a symbolic coding of irrational rotation - visualized here as a point traveling clockwise along a circle and recording whether it lies in the upper or lower half.

It’s important to note that even <img src="images/part4_sink.svg" alt="$\sin\left( k \right)$"> - despite its simple appearance - produces a fractal-like binary sequence. This is because each step advances by exactly 1 radian, and since <img src="images/part4_2pi.svg" alt="$2 \pi$"> is the full period of the sine wave, the step size is irrational with respect to the circle. In other words, <img src="images/part4_sinkmore.svg" alt="$\sin\left( k \right) > 0$"> is equivalent to <img src="images/part4_florfrackpmod2.svg" alt="$\left\lfloor \frac{k}{\pi} \right\rfloor \bmod 2$">, which is a symbolic rotation just like the classical <img src="images/part4_florksqrt2mod2.svg" alt="$\left\lfloor k \sqrt{2} \right\rfloor \bmod 2$">.
The irrational step size is the key ingredient - not the formula's appearance.
