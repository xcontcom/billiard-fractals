...
function countpoints() {
    ...
    // Draw points for the current timerK
...
function droveLines(canvas, x, y) {
    ...
    // Static drawing for k=0
    context.fillStyle = "rgb(0,0,0)";
    context.fillRect(0, 0, canvas.width, canvas.height);
    context.fillStyle = "rgb(255,255,255)";
    for (let xa = 0; xa < xend; xa++) {
        for (let ya = 0; ya < yend; ya++) {
            if (a[xa][ya] == 0) context.fillRect(xa * size, ya * size, size, size);
        }
    }
}
...
window.onload = function() {
    ...
    // Attach keydown event to document for toggling animation with spacebar
    document.addEventListener('keydown', function(evt) {
        if (evt.key === ' ' || evt.keyCode === 32) { // Spacebar
